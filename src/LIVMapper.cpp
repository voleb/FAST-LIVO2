...
void LIVMapper::initializeSubscribersAndPublishers(rclcpp::Node::SharedPtr &node, image_transport::ImageTransport &it_)
{
  ...
  if (p_pre->lidar_type == AVIA) {
    sub_pcl = this->node->create_subscription<livox_interfaces::msg::CustomMsg>(lid_topic, 200000, std::bind(&LIVMapper::avia_pcl_cbk, this, std::placeholders::_1));
  }
  else if (p_pre->lidar_type == LIVOX) {
...
void LIVMapper::avia_pcl_cbk(const livox_interfaces::msg::CustomMsg::ConstSharedPtr &msg_in)
{
  if (!lidar_en) return;
  mtx_buffer.lock();
  livox_interfaces::msg::CustomMsg::SharedPtr msg(new livox_interfaces::msg::CustomMsg(*msg_in));
